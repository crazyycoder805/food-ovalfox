...
if (isset($_POST['username'])) { 
    if (!empty($_POST['password']) && !empty($_POST['username'])) {
        ...
        $user = $pdo->read('admin', ['username'=>$_POST['username'], 'password'=>$_POST['password']]);
        
        if (!empty($user)) {
            

            $_SESSION['food_project_admin_id'] = $user[0]['id'];
            $_SESSION['food_project_admin_username'] = $user[0]['username'];
